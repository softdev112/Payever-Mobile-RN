...
public class WebViewExActivityCallbacks extends ActivityCallbacks {
  ...
  public void onActivityResult(int requestCode, int resultCode, Intent data) {
    if (requestCode == REQUEST_FILE_CODE) {
      if (Build.VERSION.SDK_INT < Build.VERSION_CODES.LOLLIPOP) {
        onActivityResultKitKat(resultCode, data);
      } else {
        onActivityResultLollipop(resultCode, data);
      }
    }
  }

  private void onActivityResultKitKat(int resultCode, Intent data) {
  ...
  private void onActivityResultLollipop(int resultCode, Intent data) {
  ...
  void setFileUploadCallbackBefore5(ValueCallback<Uri> callback) {
  ...
  void setCapturedFileName(String fileName) {
